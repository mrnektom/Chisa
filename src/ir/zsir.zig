...
const ZSIRType = enum { assign, store, call, fn_decl, fn_def, ret, branch, compare, arith, cast, loop, module_init, struct_init, field_access, ptr_op, deref_op, store_ptr, alloc_op, free_op, array_init, index_access, index_store, field_store, enum_init, match_expr, enum_decl, break_stmt, continue_stmt, not_op, asm_block };
...
pub const ZSIR = union(ZSIRType) {
    ...
    pub fn deinit(self: *const @This(), allocator: std.mem.Allocator) void {
        switch (self.*) {
            ...
            .store_ptr => self.store_ptr.deinit(allocator),
    ...
    pub fn format(
        ...
        switch (self) {
            ...
            .store, .fn_decl, .fn_def, .ret, .branch, .compare, .arith, .cast, .loop, .module_init, .struct_init, .field_access, .ptr_op, .deref_op, .store_ptr, .alloc_op, .free_op, .array_init, .index_access, .index_store, .field_store, .enum_init, .match_expr, .enum_decl, .break_stmt, .continue_stmt, .not_op, .asm_block => {},
        }
    }
};
...
pub const ZSIRFnDecl = struct {
    ...
    pub fn deinit(self: *const @This(), allocator: std.mem.Allocator) void {
        ...
        allocator.free(self.retType);
        for (self.argTypes) |t| allocator.free(t);
...
pub const ZSIRFnDef = struct {
    ...
    pub fn deinit(self: *const @This(), allocator: std.mem.Allocator) void {
        ...
        for (self.argTypes) |t| allocator.free(t);
        allocator.free(self.argTypes);
        for (self.argNames) |n| allocator.free(n);
        allocator.free(self.argNames);
    }
};
...
pub const ZSIRRet = struct {
    value: ?[]const u8,
    startPos: usize = 0,
    wrapEnumName: ?[]const u8 = null,
    wrapVariantTag: ?u32 = null,
...
/// Store a value through a pointer (for capture-by-reference writes).
/// `pointer` holds an i64-encoded address; `value` is the IR name of the new value.
pub const ZSIRStorePtr = struct {
    pointer: []const u8,
    value: []const u8,
    pointeeType: []const u8,
...
pub const ZSIRAsmInput = struct {
    reg: []const u8,
    value: []const u8,
};

pub const ZSIRAsmOutput = struct {
    reg: []const u8,
    name: []const u8,
};

pub const ZSIRAsm = struct {
    inputs: []ZSIRAsmInput,
    outputs: []ZSIRAsmOutput,
    clobbers: [][]const u8,
